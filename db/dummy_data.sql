...
-- → ProductDetail 테이블은 그대로 있고, 내용만 싹 비워짐
...
-- 전체 상품(74개)에 대해, 각 상품마다 S/M/L 중 2~3개 사이즈를 랜덤 수량으로 할당하고, 
-- 상위 카테고리별 재고 총합이 60을 넘지 않도록 조절
...
-- TopCategory별 재고 총합 조회 쿼리
SELECT 
  tc.top_category_name AS 상위카테고리,
  SUM(pd.product_quantity) AS 총재고수량
FROM productdetail pd
JOIN product p ON pd.product_id = p.product_id
JOIN location l ON p.location_id = l.location_id
JOIN brand b ON l.brand_id = b.brand_id
JOIN subcategory sc ON b.sub_category_id = sc.sub_category_id
JOIN topcategory tc ON sc.top_category_id = tc.top_category_id
GROUP BY tc.top_category_name
ORDER BY 총재고수량 DESC;
