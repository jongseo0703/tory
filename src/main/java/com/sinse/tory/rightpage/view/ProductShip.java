...
public class ProductShip extends Pages {
	...
	public ProductShip(Object testmain, Object dataModificationPage) {
		...
		// 값이 차례대로 입력될 수 있도록
		IdentifierUpdateWithNameComboBox identifierUpdateWithNameComboBox = new IdentifierUpdateWithNameComboBox(box[0],
				box[1], box[2]);
	...
	public void fillProductInfo(Product selectedProduct) {
		...
		try {
			// 상품 정보 추출
			String topCategoryName = selectedProduct.getLocation().getBrand().getSubCategory().getTopCategory()
					.getTopCategoryName();
