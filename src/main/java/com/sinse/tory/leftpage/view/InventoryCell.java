...
public class InventoryCell extends JPanel {
	...
	private int shakeOffsetX = 0; // 그래픽 흔들림용 X 오프셋
	private int shakeOffsetY = 0; // 그래픽 흔들림용 Y 오프셋
			...
			try {
				for (SubCategoryBlock block : blocks) {
					...
					for (int i = 0; i < limit; i++) {
						final int fixedIndex = maxStock - 1 - currentStock; // 인덱스를 미리 고정
						final JPanel fixedPanel = createBlockPanel(block); // 패널도 미리 고정
						block.panels.add(fixedPanel);

						// Swing 컴포넌트는 EDT에서만 수정해야 함
						SwingUtilities.invokeLater(() -> {
							remove(slots[fixedIndex]); // 고정된 인덱스로 안전하게 remove
							slots[fixedIndex] = fixedPanel; // 슬롯 배열 업데이트
							add(fixedPanel, fixedIndex); // 해당 위치에 add
							revalidate();
							repaint();
						});
	...
	private void startShakeAnimation() {
		...
		shakeTimer = new Timer(50, e -> {
			if (isShaking) {
				shakeOffsetX = random.nextInt(5) - 2; // -2~+2
				shakeOffsetY = random.nextInt(5) - 2;
				repaint(); // 그래픽만 다시 그림
			}
		});
